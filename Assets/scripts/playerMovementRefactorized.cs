...
public class playerMovementRefactorized : MonoBehaviour {
    ...
    // Apex bonus (movimiento m�s sensible en el apex)
    [Header("Apex Bonus")]
    ...
    // Supresi�n de input a pared tras wall-jump
    ...
    // Fall ramp context (cambiar gravedad en ca�da seg�n origen)
    ...
    // Velocidad base din�mica en suelo + apex mults
    ...
    // Intenci�n de drop (S presionado)
    ...
    private void Update() {
        ...
        // Cancelaci�n del downward dash con salto
        ...
        if (!consumedByDrop && !isDownwardHoldDash) {
            if (jumpDown) {
                ...
                // Buffer normal: NO lo armamos si est�s dashing o si S est� presionado en aire
                if (!isDashing && (!sHeld || isOnWall || isGrounded)) {
                    jumpBufferTimer = jumpBufferTime;
                }

                // Buffer para saltar al terminar el dash: SOLO si est�s dashing
    ...
    private void FixedUpdate() {
        // El dash mueve por corrutina; ac� solo aplicamos f�sica normal
        if (isDashing) return;
    ...
    private void ReadHorizontalInput() {
        ...
        // Resolver conflicto A+D por "�ltima tecla" (lastPressedDir)
        if (leftHeld && rightHeld) moveDir = lastPressedDir;
        else if (leftHeld) moveDir = -1;
        else if (rightHeld) moveDir = 1;
        else moveDir = 0;

        // Supresi�n del input hacia la pared tras wall-jump
        if (wallInputSuppressTimer > 0f && suppressedWallSide != WallSide.None) {
            ...
            // Si ambas teclas y la suprimida coincide con la �ltima, invierte
    ...
    private bool CheckGroundRayAt(Vector2 localOffset) {
        ...
        if (hit.collider != null) {
            // Ignora la one-way que est�s atravesando ahora mismo
        ...
        // 2) Sin m�scara: acepta por tag de suelo o por one-way (si se trata como suelo)
    ...
    private void UpdateWallDetectionAndStates(float dt) {
        ...
        // Resoluci�n del lado de pared preferido
    ...
    private void HandleJumpInput_ImmediatePressHold(bool jumpDown, bool jumpUp) {
        ...
        // Consumir buffer normal en el primer frame v�lido
    ...
    // Helper com�n para iniciar un salto (ground-like o doble)
    private void BeginJumpCommon(bool consumeDoubleJump) {
        ...
        // C�lculo de impulso inicial y fase de hold (igual que antes)
    ...
    private void DoWallJump() {
        ...
        // Supresi�n de input hacia pared si correspond�a
        if (towardWall) {
    ...
    private void ApplyMarioStyleGravities() {
        // Excepciones: downward-hold dash gestiona su ca�da aparte
        ...
        // Ca�da con rampa (FromJump o FromDash)
    ...
    // Distancia segura permitida (cast contra colisi�n, con skin)
    private float ComputeDashAllowedDistance(Vector2 dir) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // Si vamos hacia arriba, ignorar OneWay como obst�culo
    ...
    private bool TryConsumeDashJumpBuffer() {
        ...
        // Si est�s sobre OneWay e intentando drop (S), no consumas
    ...
    private bool TryDashNow() {
        ...
        if (isGrounded) {
            // En suelo: solo horizontal. Si no hay input, usa la orientaci�n actual.
    ...
    private IEnumerator DownwardHoldDashRoutine(Vector2 dir) {
        ...
        while (true) {
            ...
            // Cast frontal para parar con skin al tocar suelo/obst�culo
    ...
    private IEnumerator DropThroughRoutine(Collider2D platform, float duration) {
        ...
        // Ignora colisi�n con esa plataforma un rato
        ...
        // Peque�a tolerancia para que el raycast no la detecte inmediatamente
